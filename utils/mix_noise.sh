...
for f in $(find $SPEECH_DIR -name '*.wav'); do 
	...
	# merge and trim
	sox -m temp_speech.wav temp_noise2.wav temp_mixed.wav trim 0 `soxi -D temp_speech.wav`  # vol ratio 1:1
	# sox -m -v 0.75 temp_speech.wav -v 0.25 temp_noise2.wav temp_mixed.wav trim 0 `soxi -D temp_speech.wav`
